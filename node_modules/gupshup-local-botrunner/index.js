...
function updateCallbackBot(botName, apikey, request, devid) {
    ...
    ngrok.connect(
        ...
        , function (err, url) {
            if (url) {
                console.log(chalk.bold.yellow(`Initiating Local Server. NgRok Connect Successful.`));
                localServer.init();
                console.log(chalk.bold.yellow(`Setting Up Callback URL => ${chalk.cyan(url)}`));
                ...
                console.log(chalk.bold(`Callback URL Pushing to => ${chalk.cyan(callbackUrlSetOptions.url)} with value ${chalk.cyan(callbackUrlSetOptions.form.url)}`));
                request(callbackUrlSetOptions, function (error, response, body) {
                    ...
                    if (response.statusCode >= 300) {
                        ...
                        console.log(boxen(`
Server Started Successfully. Please Type In Any Proxy Bot ${chalk.bold.underline.green(`Proxy ${botName}$dev${devid}`)} to chat with the bot.
For more information on Proxy Bot You Can Visit ${chalk.bold.underline.blue("https://www.gupshup.io/developer/gupshupproxybot")}
                        `, {
                            padding    : 1,
                            margin     : 1,
                            borderStyle: 'round',
