...
const {MongoProperties, GSMongoDBHandler} = require("./mongodbHandler.js");

var db = {};

let currentProperties = new MongoProperties();
...
async function setupDb() {
    if (currentProperties.shouldUseMongo()) {
        await setupMongoDB();
    } else {
        setupDiskDB();
    }
...
async function setupMongoDB() {
    let mongoDBHandler = await new GSMongoDBHandler();

    db.get = function (params, callback) {
        mongoDBHandler.get(params.key).then(
            data => {
    ...
    db.put = function (params, value, callback) {
        mongoDBHandler.put(params.key, value).then(data => {
            callback(null, data);
        });
    };
}
...
function executeFunction(context, event, res, botenv) {
    // setupLogs(event.botname);
    setupSimpleDb(context, event, botenv);
...
function setupSimpleDb(botcontext, botevent, botenv) {
    var simpledb = {};
